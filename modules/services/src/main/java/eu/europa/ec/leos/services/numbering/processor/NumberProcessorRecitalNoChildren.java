...
public class NumberProcessorRecitalNoChildren extends NumberProcessorAbstract {

    private static final Logger LOG = LoggerFactory.getLogger(NumberProcessorRecitalNoChildren.class);

    @Autowired
    public NumberProcessorRecitalNoChildren(MessageHelper messageHelper, NumberProcessorHandler numberProcessorHandler) {
    ...
    public boolean canRenumber(String elementName, Node element, boolean renumberChildren) {
        if (RECITAL.equals(elementName) && !renumberChildren) {
            return true;
        }
        return false;
    }
