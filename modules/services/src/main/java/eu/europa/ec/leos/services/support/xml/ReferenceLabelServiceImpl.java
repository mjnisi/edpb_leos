...
abstract class ReferenceLabelServiceImpl implements ReferenceLabelService {
    ...
    public Result<String> generateLabel(List<Ref> refs, String sourceDocumentRef, String sourceRefId, byte[] sourceBytes, byte[] targetBytes, String targetDocType, boolean withAnchor) {
        try {
            Document targetDocument = createDocument(targetBytes);
            TreeNode targetTree = createTree(targetDocument, null, refs);
            ...
            Document sourceDocument = createDocument(sourceBytes);
            TreeNode sourceTree = createTree(sourceDocument, null, Arrays.asList(new Ref("", sourceRefId, sourceDocumentRef, null)));
            TreeNode sourceNode = TreeHelper.find(sourceTree, TreeNode::getIdentifier, sourceRefId);

            List<TreeNode> commonNodes = findCommonAncestor(sourceDocument, sourceRefId, sourceDocumentRef, targetTree);
    ...
    @Override
    public Result<String> generateSoftMoveLabel(Ref ref, String sourceRefId, byte[] sourceBytes, String attr,
                                                String sourceDocumentRef) {
        Result<String> labelResult = generateLabel(Arrays.asList(ref), sourceBytes);
