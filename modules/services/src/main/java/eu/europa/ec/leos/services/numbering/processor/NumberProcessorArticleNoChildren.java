...
public class NumberProcessorArticleNoChildren extends NumberProcessorAbstract {

    private static final Logger LOG = LoggerFactory.getLogger(NumberProcessorArticleNoChildren.class);

    @Autowired
    public NumberProcessorArticleNoChildren(MessageHelper messageHelper,
                                            NumberProcessorHandler numberProcessorHandler) {
    ...
    public boolean canRenumber(String elementName, Node element, boolean renumberChildren) {
        if (ARTICLE.equals(elementName) && !renumberChildren) {
            return true;
        }
        return false;
    }
