...
define(function leosAnnexListPluginModule(require) {
    ...
    CKEDITOR.plugins.leosAnnexList = {
        ...
        listToArray: function (listNode, database, baseArray, baseIndentLevel, grandparentNode) {
            ...
            for (var i = 0, count = listNode.getChildCount(); i < count; i++) {
                ...
                if (_isAnnexList(listNode)) {
                    ...
                    for (var j = 0, itemChildCount = listItem.getChildCount(), child; j < itemChildCount; j++) {
                        child = listItem.getChild(j);
                        if (child.type == CKEDITOR.NODE_ELEMENT && listNodeNames[child.getName()] && !(_isAnnexList(itemObj.grandparent)))// Note the recursion here, it pushes inner list items with// +1 indentation in the correct order.
        ...
        arrayToList: function( listArray, database, baseIndex, paragraphMode, dir ) {
            ...
            while ( 1 ) {
                ...
                if (item.element.type == CKEDITOR.NODE_TEXT) {
                    currentIndex++;
                } else {
                    ...
                    if (item.indent == indentLevel) {
                        ...
                        if ((!rootNode || listArray[currentIndex].parent.getName() != rootNode.getName())) {
                            if (listArray[currentIndex].element.getName() === 'ol' && listArray[currentIndex].grandparent.getAttribute('data-akn-name') === 'aknAnnexList') {
                        ...
                        if (listArray[currentIndex].indentThisElement) {
                            if (_isAnnexOrderedList(currentListItem) && listData.listNode.getChildCount() == 1) {
                            ...
                            if (_isAnnexOrderedList(currentListItem.getParent()) && listData.listNode.getChildCount() == 1 && currentListItem.findOne('ol')) {
                                var olElement = listData.listNode.getChildren().toArray()[0];
                                for (var i = 0; i < olElement.getChildCount(); i++)
                        ...
                        var leosAnnexList = _isAnnexList(item.grandparent);
                        ...
                        for ( i = 0; i < count; i++ ) {
                            ...
                            // Block content goes directly to the current list item, without wrapping.
                            else if ((child.type == CKEDITOR.NODE_ELEMENT && child.isBlockBoundary()) || (_isAnnexList(item.grandparent))) {
                                ...
                                if(!leosAnnexList && child.type == CKEDITOR.NODE_ELEMENT && !child.is('br')){
                                    currentListItem.append( child.clone( 1, 1 ) );
                                }else{
                                    ...
                                    for (var i = 0; i < childSiblings.length; i++) {
                                        if (childSiblings[i].type === CKEDITOR.NODE_ELEMENT && _isAnnexOrderedList(childSiblings[i])) {
                        ...
                        if ( (currentListItemName == 'div' || currentListItemName == 'p') && !leosAnnexList) {
    ...
    function _isAnnexOrderedList(element){ return element && 'aknAnnexOrderedList' === element.getAttribute('data-akn-name'); }

    function _isAnnexList(element) { return element && 'aknAnnexList' === element.getAttribute('data-akn-name'); }
