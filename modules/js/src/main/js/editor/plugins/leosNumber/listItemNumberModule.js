...
define(function listItemNumberModule(require) {
    ...
    function _isPoint(element) {
        var isLi = leosPluginUtils.getElementName(element) === HTML_POINT;
        var crossheadingAttr = element.getAttribute('data-akn-crossheading-type');

        if (element && isLi && (crossheadingAttr == null || crossheadingAttr != 'list')) {
            return true;
        } else {
            return false;
        }
