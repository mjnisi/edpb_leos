...
abstract class AnnexScreenImpl extends VerticalLayout implements AnnexScreen {
    ...
    void init() {
        ...
        refreshNoteButton();
        refreshButton();
        searchDelegate = new SearchDelegate(searchButton, annexLayout, messageHelper, eventBus, annexContent);
    ...
    public void setPermissions(DocumentVO annex, boolean isClonedProposal){
        ...
        actionsMenuBar.setDownloadCleanVersionVisible(instanceTypeResolver.getInstanceType().equals(InstanceType.COUNCIL.name()));
        actionsMenuBar.setSaveVersionVisible(enableUpdate);
        actionsMenuBar.setDownloadVersionVisible(isClonedProposal);
        actionsMenuBar.setDownloadVersionWithAnnotationsVisible(isClonedProposal);
        ...
        if(enableUpdate) {
            if(leosEditorExtension == null) {
                leosEditorExtension = new LeosEditorExtension<LeosDisplayField>(annexContent, eventBus, cfgHelper, structureContextProvider.get().getTocItems(), structureContextProvider.get().getNumberingConfigs(), null, getDocuments(annex), annex.getMetadata().getInternalRef());
            }
    ...
    public void initAnnotations(DocumentVO annex, String proposalRef, String connectedEntity) {
        annotateExtension = new AnnotateExtension<>(annexContent, eventBus, cfgHelper, null, AnnotateExtension.OperationMode.NORMAL, false, true, proposalRef, connectedEntity);
    }
