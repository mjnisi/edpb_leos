...
module.exports = class Guest extends Delegator
  ...
  visibleGuideLinesPrevState: true #LEOS Change
  annotationPopupStatusPrevState: true #LEOS Change
  ...
  _connectAnnotationUISync: (crossframe) ->
    ...
    crossframe.on 'LEOS_setVisibleGuideLines', (state, storePrevState) =>
      this.setVisibleGuideLines(state, storePrevState)

    crossframe.on 'LEOS_restoreGuideLinesState', () =>
      this.restoreGuideLinesState()
  ...
  anchor: (annotation, promiseChain, addToPromiseChain) ->
    ...
    removeRangeIgnoringPrefix = (anchor) ->
      if not anchor.range?
        return anchor
    
      anchorLeosSelector = (anchor.target.selector ? []).find((s) => s.type == 'LeosSelector')
      if not anchorLeosSelector?
        return anchor
      
      charactersToCompare = 5
      rangeStartContainerText = anchor.range.startContainer.textContent
      previousRangeCharacters = rangeStartContainerText.substring(anchor.range.startOffset - charactersToCompare, anchor.range.startOffset)
      if previousRangeCharacters == ''
        return anchor
      
      selectorPreviousCharacters = anchorLeosSelector.prefix.substring(anchorLeosSelector.prefix.length - charactersToCompare)
    ...
    # Remove the range of an annotation anchor if the "exact" text does not match 100%.
    removeRangeWithoutExactTextMatch = (anchor) ->
      if not anchor.range?
        return anchor

      anchorTextSelector = (anchor.target.selector ? []).find((s) => s.type == 'TextQuoteSelector')

      if not anchorTextSelector?
        return anchor

      rangeText = anchor.range.toString()
      anchorText = anchorTextSelector.exact

      if not (anchorText == rangeText)
        anchor.range = null
    ...
    if addToPromiseChain
      ...
      for target in annotation.target when target not in anchoredTargets
        ...
        .then(removeRangeWithoutExactTextMatch)
        .then(highlight)
        anchors.push(anchor)
  ...
  createAnnotation: (annotation = {}) ->
    ...
    selectors.then((selectorsValue) -> self.addSucceedingAndPrecedingText(annotation, ranges[0], selectorsValue[0])) if annotation.$suggestion
  ...
  addSucceedingAndPrecedingText: (annotation, range, selectors) ->
    return if not range or not selectors
    textInCommonAncestor = range.commonAncestorContainer.innerText
    if not textInCommonAncestor
      textInCommonAncestor = range.commonAncestorContainer.parentElement.innerText
    leosSelector = selectors.find((s) -> s.type == 'LeosSelector')
    if leosSelector
...
#    LEOS change 3632
  ...
  setVisibleGuideLines: (shouldShowGuideLines, storePrevState) ->
    if storePrevState
      @visibleGuideLinesPrevState = @visibleGuideLines
    @visibleGuideLines = shouldShowGuideLines

  restoreGuideLinesState: () ->
    @visibleGuideLines = @visibleGuideLinesPrevState
