...
REQUEST_TYPES = ["SecurityToken","UserPermissions","MergeSuggestion","MergeSuggestions","DocumentMetadata","SearchMetadata"]
...
module.exports = class HostBridgeManager
  ...
  constructor: (hostBridge, crossframe) ->
    ...
    if (@crossframe? and @hostBridge?)
      ...
      self.hostBridge.requestFilteredAnnotations = (callback) ->
        self.crossframe.call('LEOS_requestFilteredAnnotations', callback)
      self.crossframe.on "LEOS_responseFilteredAnnotations", (annotations) =>
        self.hostBridge.responseFilteredAnnotations(annotations)
